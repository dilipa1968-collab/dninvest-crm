...
function renderEqTable(){
  ...
  rows.forEach(c=>{
    ...
    h+=`<tr class="${rowCls}"${rowStyle}${comebackTag?` title="Comeback trade — ${comebackTag==='blue'?'~1':comebackTag==='green'?'~3':'~6'}+ month gap"`:''}>${BULK.td('eq',c.id)}
      <td><span style="display:inline-block;min-width:60px">${c.code||'—'}</span>${(_mfPanSet.has(String(c.pan||'').trim().toUpperCase())||_mfMobileSet.has(String(c.mobile||'').trim()))?`<span title="MF RM: ${escapeHtml(_mfRmFor(c))}" style="font-size:.62rem;background:#0d9488;color:#fff;border-radius:4px;padding:0 4px;font-weight:700;vertical-align:middle;cursor:help">M</span>`:''}</td>
      <td style="font-weight:600;cursor:context-menu" oncontextmenu="showClientSeminarMenu(event,'${c.id}','equity')" title="Right-click → Add to Seminar">${c.name}${(c.asset_value>=500000)?'<span title="HNI — Asset Value ≥ ₹5L" style="margin-left:4px;font-size:.65rem;background:#7c3aed;color:#fff;border-radius:4px;padding:0 4px;font-weight:700;vertical-align:middle;cursor:help">H</span>':''}</td>
...
function renderMfTable(){
  ...
  rows.forEach(c=>{
    ...
    h+=`<tr class="row-mf">${BULK.td('mf',c.id)}
      <td style="font-weight:600;cursor:context-menu" oncontextmenu="showClientSeminarMenu(event,'${c.id}','mf')" title="Right-click → Add to Seminar"><span style="display:inline-block;width:20px;text-align:left">${(_eqPanSet.has(String(c.pan||'').trim().toUpperCase())||_eqMobileSet.has(String(c.mobile||'').trim()))?'<span title="Also an Equity client" style="font-size:.65rem;background:#2563eb;color:#fff;border-radius:4px;padding:0 4px;font-weight:700;vertical-align:middle;cursor:help">E</span>':''}</span>${c.name}${(c.aum>=300000)?'<span title="HNI — AUM ≥ ₹3L" style="margin-left:4px;font-size:.65rem;background:#0d9488;color:#fff;border-radius:4px;padding:0 4px;font-weight:700;vertical-align:middle;cursor:help">H</span>':''}</td>
      <td><a href="tel:${c.mobile}" style="color:var(--navy);text-decoration:none">${c.mobile||'—'}</a></td>
      <td>${c.pan||'—'}</td>
      <td>${c.rm||'—'}</td>
