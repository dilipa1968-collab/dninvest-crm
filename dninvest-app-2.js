...
// ══════════════════════════════════════════
// BULK BILL ESTIMATOR — upload a day's Cash/F&O order book (.xls/.xlsx export),
// auto-group executed trades by segment, and estimate the total brokerage + statutory bill.
// ══════════════════════════════════════════
function bcRunBulkEstimate(){
  var fileInput = document.getElementById('bcBulkFile');
  var file = fileInput && fileInput.files && fileInput.files[0];
  var resultsEl = document.getElementById('bcBulkResults');
  if(!file){ resultsEl.innerHTML = '<div style="color:#dc2626;font-weight:700;font-size:.8rem">Please choose a trade file first.</div>'; return; }
  var cashMode = document.getElementById('bcBulkCashMode').value;   // 'equity_intraday' or 'equity_delivery'

  resultsEl.innerHTML = '<div style="color:var(--gray);font-size:.8rem">Reading file…</div>';
  ...
  reader.onload = function(e){
    try{
      ...
      if(!rows.length){ resultsEl.innerHTML = '<div style="color:#dc2626;font-weight:700">No data found in this file.</div>'; return; }

      var keys = Object.keys(rows[0]);
      var isFO = keys.indexOf('CONTRACT') !== -1;
      var isCash = keys.indexOf('SCRIP NAME') !== -1;
      if(!isFO && !isCash){
        resultsEl.innerHTML = '<div style="color:#dc2626;font-weight:700">Unrecognised file format — expected the Cash or F&amp;O order-book export.</div>';
        return;
      }

      // Per-segment accumulator: buy/sell qty & turnover, plus a trade count (used to approximate lot-based flat brokerage).
      var buckets = {};
      function bucket(seg){ if(!buckets[seg]) buckets[seg] = {buyQty:0,buyTurn:0,sellQty:0,sellTurn:0,trades:0}; return buckets[seg]; }

      rows.forEach(function(r){
        var status = String(r['ORDER STATUS']||'').trim();
        if(status !== 'Fully Executed') return;   // skip Rejected / Cancelled / Expired / partially unexecuted
        var bs = String(r['BUY SELL']||'').trim().toUpperCase();
        var seg;
        var qty, price;
        if(isFO){
          qty = parseFloat(r['EXECUTED QTY'])||0;
          price = parseFloat(r['EXEC PRICE'])||0;
          var instr = String(r['INSTRUMENT TYPE']||'').trim().toUpperCase();
          seg = (instr.charAt(0)==='O') ? 'options' : 'futures';   // OS/OI → Options, FS/FI → Futures
        } else {
          qty = parseFloat(r['EXEC QTY'])||0;
          price = parseFloat(r['EXEC PRICE'])||0;
          seg = cashMode;
        }
        if(!qty || !price) return;
        ...
        b.trades++;
        if(bs==='B'){ b.buyQty += qty; b.buyTurn += turnover; }
        else if(bs==='S'){ b.sellQty += qty; b.sellTurn += turnover; }
      });

      var segKeys = Object.keys(buckets);
      if(!segKeys.length){
        resultsEl.innerHTML = '<div style="color:#dc2626;font-weight:700">No Fully Executed trades found in this file.</div>';
        return;
      }

      var segLabels = { futures:'Futures (F&O)', options:'Options', equity_intraday:'Equity Intraday', equity_delivery:'Equity Delivery',
                         commodity_futures:'Commodity Futures', commodity_options:'Commodity Options' };

      var grand = {turnover:0, brokerage:0, charges:0, total:0, trades:0};
      var rowsHtml = '';
      segKeys.forEach(function(seg){
        var b = buckets[seg];
        var d = BC_SEGMENT_DEFAULTS[seg];
        var shape = BC_SEG_BROK_TYPE[seg];
        var totalTurnover = b.buyTurn + b.sellTurn;

        var brokerage;
        if(shape==='flat') brokerage = d.brokFlat * b.trades;             // approx: 1 lot per executed order row
        else if(shape==='pct_min') brokerage = Math.max(b.buyTurn*d.brokPct/100, b.buyQty*d.brokMin) + Math.max(b.sellTurn*d.brokPct/100, b.sellQty*d.brokMin);
        else brokerage = totalTurnover * d.brokPct/100;

        var txn = totalTurnover * d.txn/100;
        var sebi = totalTurnover * d.sebi/100;
        var stamp = b.buyTurn * d.stamp/100;
        var stt = (d.sttSide==='both') ? totalTurnover*d.stt/100 : b.sellTurn*d.stt/100;
        var gst = (brokerage+txn+sebi) * 0.18;
        var charges = brokerage+txn+sebi+stamp+stt+gst;
        var total = charges;   // "bill amount" = total charges for that segment (brokerage + statutory)

        grand.turnover += totalTurnover; grand.brokerage += brokerage; grand.charges += (charges-brokerage); grand.total += total; grand.trades += b.trades;

        rowsHtml += '<tr><td>'+(segLabels[seg]||seg)+' ('+b.trades+' trades)</td><td>'+bcFmt(totalTurnover)+'</td><td>'+bcFmt(brokerage)+'</td><td>'+bcFmt(charges-brokerage)+'</td><td>'+bcFmt(total)+'</td></tr>';
      });

      rowsHtml += '<tr class="bc-bulk-total"><td>Grand Total ('+grand.trades+' trades)</td><td>'+bcFmt(grand.turnover)+'</td><td>'+bcFmt(grand.brokerage)+'</td><td>'+bcFmt(grand.charges)+'</td><td>'+bcFmt(grand.total)+'</td></tr>';

      resultsEl.innerHTML =
        '<table class="bc-bulk-table"><thead><tr><th>Segment</th><th>Turnover</th><th>Brokerage</th><th>Other Charges</th><th>Total Bill</th></tr></thead>'
        +'<tbody>'+rowsHtml+'</tbody></table>'
        +'<div class="bc-bulk-note">Uses each segment\'s currently-set default Brokerage Rate. Options/Commodity Options brokerage is approximated as one lot per executed order (exact lot sizes aren\'t in this file) — treat as indicative, not final.</div>';
    }catch(err){
      resultsEl.innerHTML = '<div style="color:#dc2626;font-weight:700">Could not read this file: '+(err&&err.message?err.message:err)+'</div>';
    }
  };
